...
do
	...
	do
		...
		do
			...
			time -f "$SCENE $SAMPLE $RESOLUTION %e %M" -a -o $LOG_FILE ../../straylight -t $THREADS -w $RESOLUTION -h $RESOLUTION -f nffs/$SCENE.nff -p -o outputs/$SCENE-$SAMPLE-$RESOLUTION.png
		   	echo "$SCENE: Done"
		done
	done	
done

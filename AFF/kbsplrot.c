...
/**
 * Information for squad implementation of quaternions.
 */
typedef struct
{
...
void* KB_RotInitialize (int numKeys, RotationKey* key)
{
   ...
   {
      FromAngleAxis(&q0, key[i0].Rot.angle, key[i0].Rot.x, 
         key[i0].Rot.y, key[i0].Rot.z);
      FromAngleAxis(&q1, key[i1].Rot.angle, key[i1].Rot.x, 
         key[i1].Rot.y, key[i1].Rot.z);
      FromAngleAxis(&q2, key[i2].Rot.angle, key[i2].Rot.x, 
         key[i2].Rot.y, key[i2].Rot.z);
      FromAngleAxis(&q3, key[i3].Rot.angle, key[i3].Rot.x, 
         key[i3].Rot.y, key[i3].Rot.z);
      ...
      if ( Dot(&q1, &q2) < 0.0 )
      {
         /* q2 = -q2; */
         NegSelf(&q2);
         ToAngleAxis(&q2, &(key[i2].Rot.angle), &(key[i2].Rot.x), 
            &(key[i2].Rot.y), &(key[i2].Rot.z));
      }
      ...
      MulScal(&Tout, &logdq, out1);
      LogSelf(&prod);
      MulScal(&prod, &prod, out0);
      AddSelf(&Tout, &prod);
      ...
      MulScal(&Tin, &logdq, in0);
      LogSelf(&prod);
      MulScal(&prod, &prod, in1);
      AddSelf(&Tin, &prod);
      ...
      Sub(&arg0, &Tout, &logdq);
      MulScalSelf(&arg0, 0.5);
      /* Quaternion arg1 = 0.5 * (logdq - Tin); */
      Sub(&arg1, &logdq, &Tin);
      MulScalSelf(&arg1, 0.5);

      /* info->si[i0].a = q1 * arg0.Exp(); */	
      /* info->si[i0].b = q2 * arg1.Exp(); */
      ExpSelf(&arg0);
      Mul(&(info->si[i0].a), &q1, &arg0);
      
      ExpSelf(&arg1);
      Mul(&(info->si[i0].b), &q2, &arg1);

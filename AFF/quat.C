...
#else
    /* The above code uses 16 multiplications and 12 additions.  The
     * code below uses 12 multiplications (you might be able to avoid
     * the divisions by 2 and manipulate the floating point exponent
     * directly) and 28 additions.  For an architecture where
     * multiplications are more expensive than additions, you should do
     * the cycle count and consider using the code below.  On an Intel
...
{
   /* NOTE: Multiplication is not generally commutative, so in most cases
    * p*q != q*p. */
    ...
    * divisions by 2 and manipulate the floating point exponent
    * directly) and 28 additions.  For an architecture where
    * multiplications are more expensive than additions, you should do
    * the cycle count and consider using the code below.  On an Intel
    * Pentium, multiplications and additions cost the same, so the code
...
{
   /* If q = A * (x * i + y * j + z * k) where (x, y, z) is unit
    * length, then exp(q) = cos(A) + sin(A) * (x * i + y * j + z * k).
    * If sin(A) is near zero, use
...
{
   /* If q = cos(A) + sin(A) * (x * i + y * j + z * k) where (x, y, z)
    * is unit length, then log(q) = A * (x * i + y * j + z * k).  If

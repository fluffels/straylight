...
struct RStarBoundingBox {
	...
	bool stretch(const RStarBoundingBox<dimensions> &bb)
	{
		...
		for (std::size_t axis = 0; axis < dimensions; axis++)
		{
			...
			{
				edges[axis].first = bb.edges[axis].first - 0.001;
				ret = true;
			}
		
			if (edges[axis].second < bb.edges[axis].second)
			{
				edges[axis].second = bb.edges[axis].second + 0.001;
				ret = true;
			}
		}
  ...
  /**
   * Check whether the given ray intersects this BoundingBox.
   *
   * @param r The ray to intersect with this BoundingBox.
   *
   * @return True if the ray intersects this BoundingBox, false otherwise.
   */
	...
	double distanceFromCenter(const RStarBoundingBox<dimensions>& bb) const
	{
		...
		for (std::size_t axis = 0; axis < dimensions; axis++)
		{
			t = (edges[axis].first + edges[axis].second + 
			     bb.edges[axis].first + bb.edges[axis].second)
				 /2.0;
			distance += t*t;
		}

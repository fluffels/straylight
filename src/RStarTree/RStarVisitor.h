...
// returns true if the compared boundary intersects with the specified ray
template <typename Node, typename Leaf>
struct RStarAcceptIntersecting
{
  const Ray &m_ray;
  explicit RStarAcceptIntersecting(const Ray &ray) : m_ray(ray) {}
  
  bool operator()(const Node * const node) const 
  { 
    return node->bound.intersect(m_ray);
  }
  
  bool operator()(const Leaf * const leaf) const
  { 
    return leaf->bound.intersect(m_ray);
  }
  
  private: RStarAcceptIntersecting(){}
};
